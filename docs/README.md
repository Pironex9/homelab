...
- [35 - Cron Job Monitoring (Uptime Kuma)](./proxmox/35_Cron_Job_Monitoring_Uptime_Kuma.md) - Dead man's switch monitoring for eight cron jobs across three hosts using push monitors on the off-site Uptime Kuma, why a cron manager UI would not have caught any of the three silent failures, creating monitors by SQLite insert because Kuma has no write API, and the three gating traps: a zero-iteration loop that reports success, a healthy command that exits non-zero, and a legitimate skip that must still ping
