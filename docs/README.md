...
- [44 - Host Metrics Into The K3s Prometheus](./proxmox/44_Host_Metrics_Into_The_K3s_Prometheus.md) - Putting the Proxmox host and the docker LXC into the K3s cluster's Prometheus over the tailnet: the Netdata install that has alarmed on the thin pool the whole time into a mailbox nobody reads, the Debian patch that excludes `/mnt` from node_exporter so the four SnapRAID disks were silently absent, why `ARGS` has to go in the defaults file rather than a systemd drop-in, the textfile collector the thin pool needs because a pool is not a filesystem and its own dead man's switch, why the metadata percentage gets a lower threshold and a higher severity than the data percentage, and the second selector in one day that matched zero series while reporting healthy
- [45 - Fleet-Wide Package Updates](./proxmox/45_Fleet_Wide_Package_Updates.md) - Clearing 231 security packages across nine LXCs and the hypervisor in one sitting, and writing down the procedure that never existed: why the rollback path for a release upgrade has to be known before the first command runs, the Postgres killed by Docker's 10-second stop timeout and the one log line that proves its shutdown checkpoint still finished, dating three failed systemd units to before the updates with `ActiveEnterTimestamp`, the `latest-stable` symlink that turns an Alpine patch round into a release branch upgrade, the exact-version pin that protects Tailscale instead of breaking apk, validating a reverse-proxy config against the new binary while the old one still serves, the `.apk-new` files that would restore a WireGuard port collision if copied back, and why `fstrim -av` on the host returns nothing to the thin pool
- [46 - Pedicure Practice App](./proxmox/46_Pedicure_Practice_App.md) - The first application here that was written rather than deployed, and the ten defects that a plan, 250 tests, a fresh-eyes review of every diff and finally a real HTTPS deploy each caught after the previous stage had missed it: `executescript()` committing so the migration transaction never existed, a check-then-write that double-booked the day until every transaction began IMMEDIATE, two aware datetimes with the same tzinfo comparing as wall clock across a DST change, SQLite folding case for ASCII only so the search returned nothing for what anyone would type, floor division getting the sign of money wrong, Komodo deploying seventeen commits onto the image built at the first one, Pangolin evaluating access rules first-match-wins against what its own UI says, Let's Encrypt failing on a DNS record minutes old, a service worker scoped to the directory it was served from, and a backup section naming a backup that does not contain the data
- [47 - When One File Is Two Episodes](./proxmox/47_When_One_File_Is_Two_Episodes.md) - A 50-file season pack that cannot be mapped onto its own metadata because the release counts broadcast half-hours while every provider counts the 11-minute stories inside them: why switching from TVDB to TMDB changes nothing when both are right, the `ffprobe` duration pass that rules out whole layouts in seconds, grouping provider episodes by air date to convert segments back into slots and why three seasons matching exactly is corroboration rather than proof, the four frame-level checks that all came back empty, a verification script that invented 22 failures by letting `read` split quoted paths, the completed torrent that re-imports the whole pack after any rescan because `monitored: false` does not reach download handling, why changing its category keeps it seeding while the queue deletion alone does not hold, the host's own `127.0.0.1` that is not localhost to a containerised WebUI, a series folder whose season range gets swallowed into the title and leaves zero provider IDs, and the auth header a major version dropped underneath a key that was still valid
- [48 - When an Episode Is an Executable](./proxmox/48_When_An_Episode_Is_An_Executable.md) - Two queue items that were gigabyte-padded Windows binaries rather than episodes, and the audit that followed: why `file` and `ffprobe` are both needed to identify a fake release, the seeding that continued for hours because a blocked import is not containment, the blocklist entry proving it was the third instance and not the first, judging an indexer by whether it controls who uploads instead of by catalogue size, why the bad indexer has to be disabled before the queue is cleared, a library sweep that covered four of ten roots and returned a meaningless clean result, proving a suspect binary with `grep -a` for library identifiers and injection APIs and a `.lnk` from its LinkFlags bitfield, the TheXEM map that stops two seasons short so every remaining episode blocks on `unverifiedSceneNumbering` with no setting to disable it, the documented Cloudflare tool its own guide marks non-functional and the drop-in that shares its port and API, a compose file whose `networks:` declaration the service never actually joined, and a green `DeployStack` that deployed nothing because Compose saw an identical project already running
- [49 - Six Hours of "No Internet"](./proxmox/49_Six_Hour_NIC_Hang.md) - A TV that could not reach Jellyfin and a desktop reporting no internet, both caused by the hypervisor's onboard I219-LM wedging its transmit ring for 6 hours 15 minutes: why the router power cycle that appeared to fix it only ever dropped the link, three independent subsystems on two machines agreeing on the onset minute, the CEST-host/UTC-container split that makes the same event look two hours apart, why a single bridge port takes every LXC down while container-to-container traffic stays up and disguises a link fault as a DNS fault, 11,246 hang detections with zero adapter resets which is what turned a transient into a night, the chronic NFS timeouts 49 seconds earlier that look like a trigger and are a separate fault still running after recovery, why a successful `dig @9.9.9.9` is worthless on a LAN whose router intercepts port 53, ruling out a rogue DHCP server by the absence of lease events rather than by assumption, and why the documented TSO workaround was deliberately not applied after one occurrence in nine months of logs
