...
- **Tailscale on LXC 105 (Alpine):** Requires TUN device in `/etc/pve/lxc/105.conf` (same as LXC 109). Install via `apk add tailscale`, start with `rc-service tailscale start`, enable with `rc-update add tailscale default`. Use `--accept-dns=false` to avoid DNS conflicts - but see the next entry, that flag alone is a half-fix.
- **Updating Tailscale here: name the edge repo on the command line, never `apk upgrade` (2026-08-27):** `latest-stable/community` tops out at tailscale 1.98.5-r0, so `apk upgrade tailscale` cannot reach a current client; 1.102.3 lives only on Alpine `edge`. And a *blanket* `apk upgrade` is not a package bump but a distro jump - `alpine-release` here is 3.23.3 while latest-stable has rolled to 3.24.1, so it would pull 40+ packages including `apk-tools`, `busybox` and `openrc` underneath the running Komodo Core. The working route, used to take this container 1.98.5 -> 1.102.3, keeps `/etc/apk/repositories` untouched and moves exactly two packages: `apk add --no-cache --repository=http://dl-cdn.alpinelinux.org/alpine/edge/community tailscale=1.102.3-r0 tailscale-openrc=1.102.3-r0`, then `rc-service tailscale restart` (mandatory - apk swaps the binary but leaves the old daemon running). Run it with `--simulate` first and confirm nothing but those two packages moves. Afterwards `apk policy tailscale` shows the installed 1.102.3-r0 with no repository offering it, and `apk upgrade` leaves it alone since apk does not downgrade by default. Full procedure and the two things it disturbs: `docs/proxmox/31_Tailscale_Port_Collision_DNS_Audit.md`.
- **`accept-dns=false` plus no `pct` nameserver = every git fetch fails (2026-08-08):** The container had no `nameserver:` line in `/etc/pve/lxc/105.conf`, so at start Proxmox copied the *host's* `/etc/resolv.conf` in - and on pve that file is owned by tailscaled, so it reads `nameserver 100.100.100.100`. With `accept-dns=false` the local tailscaled resolver has no upstream nameservers, so it answers MagicDNS and `.lan` split-DNS but `SERVFAIL`s everything else. Komodo kept running on its cached git checkout and only broke when it needed a fresh fetch: **`Failed at: Git Fetch - Could not resolve host: github.com` on every stack at once.** Diagnostic that separates it from a Komodo fault in one command: `nslookup github.com 192.168.0.111` succeeds from the same container while the default resolver fails. Fix: `pct set 105 --nameserver '192.168.0.111 1.1.1.1'` on pve (survives restarts), then write the same two lines into `/etc/resolv.conf` on 105 for the running container. **Containers do not pick this up on their own** - Docker's embedded resolver at `127.0.0.11` caches the host upstreams from container start, so `docker restart komodo-core-1 komodo-periphery-1` is required. Verify with `docker exec komodo-core-1 git ls-remote https://github.com/Pironex9/homelab HEAD`. Same failure mode and same fix as LXC 109 in June 2026.
