...
- **Do not run a blanket `apk upgrade` here to update Tailscale (2026-08-27):** the client sits on 1.98.5-r0 while Tailscale stable is 1.102.3, and `apk upgrade tailscale` will not close that gap - `latest-stable/community` has no newer build, 1.102.3 lives only on Alpine `edge`. A full `apk upgrade` is not a package bump either: `alpine-release` here is 3.23.3 while `latest-stable` has already rolled to 3.24.1, so the command turns into a 3.23 -> 3.24 distro jump across 40+ packages including `apk-tools`, `busybox` and `openrc`, underneath the running Komodo Core. Left on 1.98.5 deliberately - four minor versions of lag costs nothing against Tailscale's backward-compatibility promise. If it ever has to move, the clean route is the static tarball from `pkgs.tailscale.com` with a hand-written OpenRC service, not `edge`. Full reasoning in `docs/proxmox/31_Tailscale_Port_Collision_DNS_Audit.md`.
