...
- **The remaining hole is the router, not the hosts:** the Archer C6 DHCP hands out `192.168.0.111` *and* `192.168.0.1` as DNS. On any systemd-resolved client that is enough to lose the filtering - on Nobara, `resolvectl status` showed `DNS Servers: 192.168.0.111 192.168.0.1` with `Current DNS Server: 192.168.0.1`, so it had silently picked the router. This affects every DHCP client, phones and laptops included. Clear Secondary DNS in the router's DHCP settings; the rule was already written down (see the DHCP entry above) but was not actually in force.
