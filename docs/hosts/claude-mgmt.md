...
- **Workspace:** `/root` mounted whole at `/config/workspace/root` (single parent bind mount - covers every project directory under `/root`; standard practice over per-project mounts, since Docker can't merge multiple host dirs into one path anyway). Default editor workspace opens at `/config/workspace/root/homelab`
...
- **Serve config can vanish and nothing notices.** On 2026-08-27 `https://claude-mgmt.tailc6abe2.ts.net/` was dead while the container was healthy: `tailscale serve status` printed `No serve config` and the `_serve/<profile>` key in `/var/lib/tailscale/tailscaled.state` was an empty `{}`. The state file's mtime said `Aug 15 11:14`, so it had been broken for roughly twelve days - the 1.102.2 -> 1.102.3 auto-update on 08-26 only restarted `tailscaled` on an already-empty config. What wiped it is unproven; a stray `tailscale up` or `serve reset` is the likely candidate. Fix is a re-run of `tailscale serve --bg 8443`. The Uptime Kuma monitor pointed at `http://100.98.146.14:8443` and so tested the container *behind* the proxy - it stayed green throughout. Monitor 53 now targets the HTTPS URL instead
- **Survives a power cut:** on boot Docker used to reach the port bind before `tailscale0` had its address, and the container died with `failed to bind host port 100.98.146.14:8443/tcp: cannot assign requested address` (exit 255, `unless-stopped` gave up after its retries). `/etc/sysctl.d/99-nonlocal-bind.conf` sets `net.ipv4.ip_nonlocal_bind = 1` so the bind succeeds before the interface is ready. Applied 2026-08-20 after the storm outage
...
- **A container bound to a Tailscale IP loses the boot race:** Docker starts before `tailscaled` assigns the 100.x address, the port bind fails with `cannot assign requested address`, and the restart policy exhausts its retries - so the container is `Exited (255)` long after the network is fine. Fix is `net.ipv4.ip_nonlocal_bind = 1` in `/etc/sysctl.d/`, not `After=tailscaled.service` on `docker.service`: tailscaled reporting "started" does not mean the IP is up yet.
- **After such a failure `docker start` is not enough:** the container comes up and listens internally, but `NetworkSettings.Ports` stays empty - no host port proxy. Verify with `docker inspect <name> --format '{{json .NetworkSettings.Ports}}'` and recreate the stack (`docker compose up -d --force-recreate`) instead of trusting `docker ps` showing `Up`.
...
`scripts/homelab-digest.sh` runs daily at 07:00 via root's crontab (`0 7 * * * /root/homelab/scripts/homelab-digest.sh`). It's a plain bash script (no LLM involved - see the agentos doc's "Homelab Monitoring" section for why) that SSHes into pve and the docker host to collect LVM thin-pool usage, SnapRAID sync/scrub/SMART status, LXC/VM up state, today's backup status, Docker container health, and Vaultwarden package drift, then posts a summary to the `homelab-digest` ntfy topic (`https://ntfy.lan`, proxied via Caddy from agentos/LXC 113).

The Vaultwarden check (added 2026-08-28) compares the installed packages on LXC 103 against the Alpine repository and warns only on a difference. It is a notification rather than an unattended upgrade on purpose: that container holds every other credential in the homelab, so an upgrade that goes wrong must go wrong while somebody is watching. The `apk update` result is carried in its own OK/FAIL line, because without it a container with broken DNS - a known failure mode on these LXCs - returns an empty list and the check would report "naprakész" while actually being blind.
